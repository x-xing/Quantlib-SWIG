...
%{
    ...
    typedef boost::shared_ptr<ShortRateModel> ExtendedCoxIngersollRossPtr;
  ...
  public:
    %extend {
        ...
        static std::vector<bool> FixedReversion() {
            return QuantLib::HullWhite::FixedReversion();
        }
    }
};

%rename(ExtendedCoxIngersollRoss) ExtendedCoxIngersollRossPtr;
class ExtendedCoxIngersollRossPtr :  public boost::shared_ptr<ShortRateModel> {
  public:
    %extend{
        ExtendedCoxIngersollRossPtr(
            ...
            return new ExtendedCoxIngersollRossPtr(
                new ExtendedCoxIngersollRoss(termStructure, theta, k, sigma, x0, withFellerConstraint));
        }
        /*
        boost::shared_ptr<Lattice> tree(const TimeGrid& grid) const {
            return  boost::dynamic_pointer_cast<ExtendedCoxIngersollRossPtr>(*self)->tree(grid);
        }

        boost::shared_ptr<ShortRateDynamics> dynamics() const{
            return  boost::dynamic_pointer_cast<ExtendedCoxIngersollRossPtr>(*self)->dynamics();
        }
        */
        
        Real discountBond(Time now, Time maturity, Rate rate) const{
            return boost::dynamic_pointer_cast<ExtendedCoxIngersollRoss>(*self)->
                                            discountBond(now, maturity, rate);  
        }
        Real discountBondOption(Option::Type type,
            ...
            return  boost::dynamic_pointer_cast<ExtendedCoxIngersollRoss>(*self)->
                        discountBondOption(type, strike, maturity, bondMaturity);
        }
    }
};
